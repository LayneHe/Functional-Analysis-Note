...
求算子范数的一般方法就是这样了，通过Hölder不等式等方法进行放缩，找出 $C$ 的取值。
...
$$1\leqslant \mu\bigg\|\sum_{k=1}^n\frac{\alpha_k}{\sum_{j=1}^n|\alpha_j|}x_k\bigg\|$$
由此可见，引理成立。$\square$
...
设：$\{e_1,\cdots,e_n\}$ 是 $X$ 的一组基，对任意 $x\in X$ 有唯一的 $x=\xi_1e_1+\cdots+\xi_ne_n,\;\xi_j(j=1,\cdots,n)$ 是实数。所以 $(\xi_1,\cdots,\xi_n)\in \mathrm{R}^n$。定义：算子 $T$ 
...
上式是已经用过的套路了。齐性+三角不等式。要继续下去，我们需要继续放缩，我们采用的范数涉及平方开方的操作，我们应该有意识地用 Hölder不等式试试
...
我们注意：$(\sum_{j=1}^n\|e_j\|^2)^{\frac{1}{2}}$ 是和 $x$ 无关的常数 $\mathrm{C}$
